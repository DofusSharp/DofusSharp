...
public class SubAreasClientTest
{
    ...
    {
        IDofusDbTableClient<DofusDbSubArea> client = DofusDbClient.Beta().SubAreas();
        DofusDbSubArea value = await client.GetAsync(1);
        await Verify(value);
    }
    ...
    public async Task SubAreasClient_Should_SearchSubAreas()
    {
        ...
        DofusDbSubArea[] results = await client.MultiQuerySearchAsync(query).ToArrayAsync();
