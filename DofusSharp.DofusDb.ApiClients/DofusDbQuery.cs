...
public static class DofusDbQuery
{
    ...
    public static DofusDbQueryProvider Production(Uri? referrer = null) => new(DofusDbApiClient.Production(referrer));

    /// <summary>
    ///     Create a query provider for the DofusDb API that connects to the beta API.
    /// </summary>
    /// <param name="referrer">The referer header to include in requests to the API.</param>
    public static DofusDbQueryProvider Beta(Uri? referrer = null) => new(DofusDbApiClient.Beta(referrer));
}
