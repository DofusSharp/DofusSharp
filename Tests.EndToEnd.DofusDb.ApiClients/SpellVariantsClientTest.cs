...
public class SpellVariantsClientTest
{
    ...
    {
        IDofusDbTableClient<SpellVariant> client = DofusDbClient.Beta(Constants.Referrer).SpellVariants();
        SpellVariant value = await client.GetAsync(16);
        await Verify(value);
    }
    ...
    public async Task SpellVariantsClient_Should_SearchSpellVariants()
    {
        ...
        SpellVariant[] results = await client.MultiQuerySearchAsync(query).ToArrayAsync();
