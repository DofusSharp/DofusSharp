...
public class SuperAreasClientTest
{
    ...
    {
        IDofusDbTableClient<DofusDbSuperArea> client = DofusDbClient.Beta().SuperAreas();
        DofusDbSuperArea value = await client.GetAsync(0);
        await Verify(value);
    }
    ...
    public async Task SuperAreasClient_Should_SearchSuperAreas()
    {
        ...
        DofusDbSuperArea[] results = await client.MultiQuerySearchAsync(query).ToArrayAsync();
